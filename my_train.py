...
from NET import CE_Loss, Dice_loss, LossHistory
...
for i in range(epoch):  # 第i个epoch
        ...
        for j in range(batch_num_train):  # 第j个batch
            ...
            if (batch + 1) * batch_size < len(train_lines):  # 别超过索引才能传进来训练
                ...
                with torch.no_grad(): # 数据转tensor
                    ...
                    labels = labels.view(-1,6,256,256)
                ...
                beta = [0.043**2, 0.059**2, 0.065**2, 0.176**2, 0.167**2,  0.099**2]
                for temp in range(6):
    ...
    torch.save(model.state_dict(), 'E:/人工林识别论文/时空数据融合代码/model_data/estarfm_我的模型_加上ssim_去掉损失自适应.pth')
